...
def chaser(paddle_frect, other_paddle_frect, ball_frect, table_size):
    ...
    if chaser.ai.ball_towards:
      killit = (other_paddle_frect.pos[1] + other_paddle_frect.size[1] / 2 > chaser.ai.TABLE_CENTER)
      beta = math.atan(1 - ((vel[1] / vel[0]) * (abs(paddle_frect.pos[0] - other_paddle_frect.pos[0]) / ((-table_size[1] * killit) - prediction))))
      beta = min(-.5 , beta)
      beta = max(.5, beta)
    ...
    if not (paddle_hit_point > ((paddle_frect.size[1] / 8)) and paddle_hit_point < ((7 * paddle_frect.size[1] / 8))):
      if prediction < chaser.ai.TABLE_CENTER:
          paddle_hit_point = 1 * paddle_frect.size[1] / 8
      elif prediction > chaser.ai.TABLE_CENTER:
          paddle_hit_point = (7 * paddle_frect.size[1] / 8) 
      else:
          paddle_hit_point = paddle_frect.size[1] / 2

    if paddle_hit_point + paddle_frect.pos[1] < prediction:
     return 'down'
    elif paddle_hit_point + paddle_frect.pos[1] > prediction: 
     return 'up'
    else:
     return 'nothing'
