...
# The pygame clock is an object that keeps track of time and controls the framerate
# RESOLUTION of time == 10 (ms)
clock = pygame.time.Clock()

class fRect:
    '''
    pygame's Rect class can only be used to represent whole integer vertices,
    so we create a rectangle class that can have floating point coordinates
    to represent the moving objects in the game.
    '''

    def __init__(self, pos, size):
        '''(fRect, tuple, tuple) -> NoneType

        Initalize the fRect rectange with a postition pos and size size.
        '''
    ...
    def move(self, x, y):
        ...
        Return a new fRect with different pos attribute moved by x and y.
        '''
        return fRect((self.pos[0]+x, self.pos[1]+y), self.size)

    def move_ip(self, x, y):
        ''' (fRect, num, num) -> NoneType

        moves the current fRect's instance pos by x and y vector.
        '''
        self.pos = (self.pos[0] + x, self.pos[1] + y)

    def get_rect(self):
        ''' (fRect) -> Rect
        Return a Rect reprensentation of the fRect.
        '''
    ...
    def intersect(self, other_frect):
        ...
        Returns True if fRect self has an intersection with fRect other,
        else return False.
        '''
        ...
        for i in range(2):
            if self.pos[i] < other_frect.pos[i]: # projection of self begins to the left
                if other_frect.pos[i] >= self.pos[i] + self.size[i]:
                    return 0
            elif self.pos[i] > other_frect.pos[i]: # projection begins to the right
                if self.pos[i] >= other_frect.pos[i] + other_frect.size[i]:
                    return 0
            # last case have same poitions
        return self.size > 0 and other_frect.size > 0


class Paddle:
    def __init__(self, pos, size, speed, max_angle,  facing, timeout):
        ''' (Paddle, tuple, tuple, num, num, num, num) -> NoneType

        Initialzes a padddle with postion pos, size, speed, the side of the
        board facing (1 or 0 depends on side), the max_angle of ball bounce off 
        paddle, and the timeout duration for the paddle movement detection.
        '''

        # make a paddle centered in pos and of right size size
    ...
    def factor_accelerate(self, factor):
        '''Accelerate the paddle speed by amount'''
        self.speed = factor*self.speed


    def move(self, enemy_frect, ball_frect, table_size):
        ''' (fRect, fRect, Ball, tuple) -> NoneType

        Moves the instance of the paddle up or down depending on the input
        given by the AI. Further controls the speed depending on the distance
        of the paddle from the sides of the table. 
        '''
        ...
        if direction == "up":
            ...
            # increase y coord position
            self.frect.move_ip(0, self.speed)

        # the distance to the bottom of the table
        to_bottom = (self.frect.pos[1]+self.frect.size[1])-table_size[1]

        # the speed of the paddle depends on how far away it is from the top / bottom
        # why?
        if to_bottom > 0:
    ...
    def get_face_pts(self):
        ...
        Returns the tuple ((top-left / top - right corner), (bottom - left / bottom-right corner)).
        This possibly represnts the bouncing face of the paddle.
         __
        |  |
        |  |
        |  |
        |  |    <------- * Ball hits bouncing face
        |  | 
        |  |
        |__|
    ...
    def get_angle(self, y):
        ...
        Returns the angle at which the ball is bounced away?
        '''

        # get the center paddle y coordinate
        center = self.frect.pos[1]+self.size[1]/2
        # express the difference between y and center y coordinate in terms of length of paddle
        rel_dist_from_c = ((y-center)/self.size[1])
        # we do not want y to be bigger than 0.5 paddles size away
...
class Ball:
    ''' The ball object in pong'''
    def __init__(self, table_size, size, paddle_bounce, wall_bounce, dust_error, init_speed_mag):
        ''' (Ball, tuple, tuple, num, num, num, num)

        Initialises a ball on a table size with size size, paddle_bounce and wall_bounce
        coefficients, a dust_error, and initalised speed magnitude.
        '''

        # choose a random angle and a corresponding speed to give the ball
        ...
        # the ball starts at the center of the table.
        pos = (table_size[0]/2, table_size[1]/2)
        #pos = (table_size[0]/2 - 181, table_size[1]/2 - 105)

        '''initalise all the attributes of the ball
        frect: the position and size fRect
        speed: the speed tuple
        size: the dimesions of the ball
        prev_bounce: holds the last bounced paddle object 
        '''
        self.frect = fRect((pos[0]-size[0]/2, pos[1]-size[1]/2), size)
        self.speed = speed
        self.size = size
    ...
    def get_speed_mag(self):
        """ (Ball) -> float
        Returns the norm of the speed vector.
        """
        return math.sqrt(self.speed[0]**2+self.speed[1]**2)

    def factor_accelerate(self, factor):
        """ (Ball, int) -> NoneType
        Multiplies the speed vector by factor factor.
        """
    ...
    def move(self, paddles, table_size):
        """ (Ball, tuple of Paddle, tuple) -> NoneType

        Outlines the mechanics of the ball movement and moves the ball according
        to those mechanics.
        """
...
def directions_from_input(paddle_rect, other_paddle_rect, ball_rect, table_size):
    ''' (Paddle, Paddle, Ball, tuple table size) -> str or None

    Returns 'up' or 'down' depending on the keys pressed by the player.
    If no keys pressed return None.
    '''
...
def timeout(func, args=(), kwargs={}, timeout_duration=1, default=None):
    '''
    This code is too complicated for me to fully comprehend, but it seems like
    a simultaneous process that either returns one of the following
        None
        func(*args, **kwargs)
    '''
    '''From:
...
def render(screen, paddles, ball, score, table_size):
    ''' Renders the game surface and refreshes it'''
    screen.fill(black)
...
def check_point(score, ball, table_size):
    ''' Updates the score and restarts the ball if a paddle doesn't catch it'''
...
def game_loop(screen, paddles, ball, table_size, clock_rate, turn_wait_rate, score_to_win, display):
    ''' The game loop, moves the paddles from human / AI input and 
    stops when there is a victory, prevents program from running more than clock_rate 80 FPS'''
    score = [0, 0]
...
def init_game():
    '''
    Initialises the game variables and runs the game.
    '''
    table_size = (440, 280)
    paddle_size = (10, 70)
    ball_size = (15, 15)
    ...
    ''' refresh the game render with .flip() and change the control of paddles from players'''
    pygame.display.flip()
    clock.tick(4)
